...
public class AtualizaPerfilController {
    ...
    public ResponseEntity<?> atualizar(@RequestBody @Valid AtualizaPerfilRequest request, @PathVariable String apelido){
        Perfil perfil = repository.findByApelido(apelido)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Não existe cadastro para este apelido"));

        perfil.atualizar(request.getNome(), request.getApelido(), request.getInstituicaoDeEnsino(), request.getEnderecoImagem());
